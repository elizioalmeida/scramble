...
from .forms import CPForm, CDForm, ESForm, ITForm, TRForm, CDForm_novo, ESForm_novo, CPForm_novo, ITForm_novo, TRForm_novo, testeForm
...
def cp_delete(request, pk):
	...
	obj.delete()
	return render(request, 'controle/cpod_list.html')

def es_delete(request, pk):
    ES_ = Escopo.objects.all().order_by('nome_escopo')
    template = loader.get_template('controle/es_cpod.html')
    ctx = { 'ES_list':ES_, }
    context = RequestContext(request, ctx)
...
def grafCP(request):
    ...
    return FileResponse(open(os.path.dirname(__file__) + '/static/reports/relGrafCP.pdf', 'r'), content_type='application/pdf')    
